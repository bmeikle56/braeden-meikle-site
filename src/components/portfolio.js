...
function AppContributions() {
  ...
  return (<div style={{display: 'grid', justifyContent: 'center'}}>
    {[geicoMeta, hiyaMeta].map((meta) => <div style={appEntryStyle}>
      <img src={meta.img} style={{margin: '0 -12px 0 0', clipPath: meta.clip, borderRadius:'20px', width: meta.imgSq.width, height: meta.imgSq.height}}/>
      <div style={{width: 8, height: 8, backgroundColor: 'white', borderRadius: '50%'}}></div>
      <pre style={{margin: '0 0 0 12px', color: 'white', fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, Roboto, Oxygen, Ubuntu, Cantarell, Open Sans, Helvetica Neue, sans-serif'}}>{meta.prefix}</pre>
      <a target='_blank' className='app-store' href={meta.link} style={{paddingRight: 4}}>{meta.desc}</a>
